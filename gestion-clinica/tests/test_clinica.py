...
class TestClinica(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.turno = Turno(
            self.paciente, self.medico, datetime(2025, 6, 16, 10, 0), "Cardiología"
        )
    ...
    def test_agendar_turno(self):
        self.clinica.agendar_turno(
            ...
            "Cardiología",
            self.turno.obtener_fecha_hora(),
        )
        self.assertTrue(
            any(
                t.__paciente__ == self.turno.__paciente__
                and t.__medico__ == self.turno.__medico__
                and t.__fecha_hora__ == self.turno.__fecha_hora__
                and t.__especialidad__ == self.turno.__especialidad__
    ...
    def test_emitir_receta(self):
        self.clinica.emitir_receta(
            self.paciente.obtener_dni(), self.medico.obtener_matricula(), ["Aspirina"]
        )
        recetas = self.clinica.__historias_clinicas__[
            self.paciente.obtener_dni()
        ].obtener_recetas()
        self.assertTrue(
            any(
                r.__paciente__ == self.paciente
                and r.__medico__ == self.medico
                and r.__medicamentos__ == ["Aspirina"]
                for r in recetas
            )
        )
    ...
    def test_emitir_receta_sin_medicamentos(self):
        with self.assertRaises(RecetaInvalidaException):
            self.clinica.emitir_receta(
                self.paciente.obtener_dni(), self.medico.obtener_matricula(), []
            )
    ...
    def test_emitir_receta_paciente_inexistente(self):
        with self.assertRaises(PacienteNoEncontradoException):
            self.clinica.emitir_receta(
                "99999999", self.medico.obtener_matricula(), ["Aspirina"]
            )

    def test_emitir_receta_medico_inexistente(self):
        with self.assertRaises(MedicoNoDisponibleException):
            self.clinica.emitir_receta(
                self.paciente.obtener_dni(), "NOEXISTE", ["Aspirina"]
            )
